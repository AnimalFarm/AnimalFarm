...
{
    namespace Integration.EasySave2
    {
        public class ItemSaveLookup
        {
            /// <summary>
            ...
            public ItemSaveLookup()
                : this(-1, 0)
            { }

            public ItemSaveLookup(int itemID, uint amount)
            {
                this.itemID = itemID;
                this.amount = amount;
            }
        ...
        public class ItemReferenceSaveLookup : ItemSaveLookup
        {
            public string referenceOfCollection;

            public ItemReferenceSaveLookup()
                : this(-1, 0, string.Empty)
            { }

            public ItemReferenceSaveLookup(int itemID, uint amount, string collectionName)
            {
                this.itemID = itemID;
                this.amount = amount;
    ...
    public abstract partial class ItemCollectionBase
    {
        ...
        public void SaveEasySave2(string fileName, params string[] additionalFields)
        {
            if (useReferences)
            {
                var l = new List<ItemReferenceSaveLookup>();
                foreach (var item in items)
                {
                    if (item.item == null)
                        l.Add(new ItemReferenceSaveLookup(-1, 0, string.Empty));
                    else
                        l.Add(new ItemReferenceSaveLookup((int)item.item.ID, item.item.currentStackSize, item.item.itemCollection.collectionName));
                }
            ...
            {
                var l = new List<ItemSaveLookup>();
                foreach (var item in items)
                {
                    if (item.item == null)
                        l.Add(new ItemSaveLookup(-1, 0));
                    else
                        l.Add(new ItemSaveLookup((int)item.item.ID, item.item.currentStackSize));
                }
        ...
        private void _LoadEasySave2References(string fileName, params string[] additionalFields)
        {
            using (ES2Reader reader = ES2Reader.Create(fileName))
            {
                ...
                ItemReferenceSaveLookup[] data = reader.ReadArray<ItemReferenceSaveLookup>("itemReferenceLookups_" + easySaveCollectionName);
        ...
        private void _LoadEasySave2(string fileName, params string[] additionalFields)
        {
            ...
            {
                // Read data from the file in any order.
                ItemSaveLookup[] data = reader.ReadArray<ItemSaveLookup>("itemLookups_" + easySaveCollectionName);
