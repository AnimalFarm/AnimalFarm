...
namespace Devdog.InventorySystem
{
    ...
    public partial class VendorTriggerer : MonoBehaviour
    {
        ...
        /// <param name="amount"></param>
        /// <returns></returns>
        protected virtual bool SellItemToVendorNow(InventoryItemBase item, uint amount)
        {
        ...
        /// <param name="item"></param>
        /// <param name="amount"></param>
        protected virtual bool BuyItemFromVendorNow(InventoryItemBase item, uint amount, bool isBuyBack = false)
        {
        ...
        protected virtual bool CanAddItemsToInventory(InventoryItemBase item, uint amount)
        {
            ...
            // First verify if we have room for all these items
            uint amountCounter = amount;
            while (amountCounter > item.maxStackSize)
            {
                item.currentStackSize = item.maxStackSize;
                amountCounter -= item.maxStackSize;
            ...
            // And the remainder
            item.currentStackSize = amountCounter;
            bool canAdd = InventoryManager.CanAddItem(item);
            item.currentStackSize = originalStackSize; // And reset
            if (canAdd == false)
                return false;

            // Alright all items can be stored in the inventory.
            return true;
        }
