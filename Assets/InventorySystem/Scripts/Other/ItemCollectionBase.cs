...
namespace Devdog.InventorySystem
{
    ...
    /// </summary>
    public abstract partial class ItemCollectionBase : MonoBehaviour
    {
        #region Delegates
        ...
        /// </summary>
        public abstract uint initialCollectionSize { get; }
        ...
        {
            var a = new GameObject(GetType().Name.ToString());
        ...
        protected virtual void FillUI()
        {
            ...
            {
                items = new InventoryUIItemWrapper[initialCollectionSize];
            ...
            {
                for (uint i = 0; i < container.transform.childCount; i++)
                {
        ...
        public void NotifyItemUsed(uint itemID, uint slot, uint amountUsed)
        {
            ...
            //{
            //    // No need for if check, because item can ONLY be used if there is one.
            //    itemsTable[itemID] -= amountUsed;
            //    //Debug.Log("Item used " + itemsTable[itemID] + " left");
        ...
        public virtual bool CanAddItem(InventoryItemBase item)
        {
            ...
            for (uint i = 0; i < items.Length; i++)
            {
                ...
                {
                    // Stack it
                    if (items[i].item.currentStackSize + item.currentStackSize <= items[i].item.maxStackSize && canPutItemsInCollection)
                    {
                        canSet = true;
                        break;
                    }
        ...
        public virtual void AddItems(IEnumerable<InventoryItemBase> items, bool repaint = true)
        {
        ...
        public virtual bool CanSetItem(uint slot, InventoryItemBase item)
        {
            ...
            if (onlyAllowTypes.Length > 0 && item != null)
            {
                bool canPlace = false;
        ...
        public virtual bool MoveItem(InventoryItemBase item, uint fromSlot, ItemCollectionBase toCollection, uint toSlot, bool clearOld, bool doRepaint = true)
        {
            ...
            bool setted = toCollection.SetItem(toSlot, item); // items[fromSlot].item
            if (clearOld && setted)
                SetItem(fromSlot, null);
        ...
        public uint GetItemCount(uint id)
        {
            ...
            foreach (var item in items)
            {
                count += (item.item != null && item.item.ID == id) ? item.item.currentStackSize : 0;
            }
